    ...
    self.watchScroll = function(e) {
      ...
      if (scrollY >= docHeight - self.options.scrollOffset && fetchOn && prevScrollY <= scrollY) {
        ...
        self.collection.fetch({
          url: self.options.url || self.collection.url,
...
ga('create', 'UA-47894616-1', 'dscvry.herokuapp.com');
ga('send', 'pageview');
/* jshint ignore:end */
/**
	...
	var masthead = $('.navbar'),
		...
		function checkMastHeadScroll() {
			...
			if((scrollTop - 25) > lastScroll && isVisible) {
				$(masthead).animate({marginTop: '-175px', queue:false}, 200);
				isVisible = false;
 ...
 * @depend /third-party/backbone/backbone-min.js
 * @depend /third-party/infiniscroll.js
 * @depend /desktop/google-analytics.js
 * @depend /desktop/facebook.js
 * @depend /desktop/navbar.js
 * @depend /desktop/scroll-events.js
 **/
...
$(function(){
  ...
  //  $(this).scrollLeft(event.currentTarget.pageYOffset/8);
  //});

  //Custom Fire events

  var fire = {}; // or App.vent depending how you want to do this
  _.extend(fire, Backbone.Events);
  ...
  var User = Backbone.Model.extend({
      defaults: function() {
        return {
          ...
          id: '1337',
          name: 'No Nameson',
          image: 'https://d3rt1990lpmkn.cloudfront.net/640/aac6bed4cf214652c88137dca9152362eb55f961'
        };
      }
    });
  ...
  var UserAlbumList = Backbone.Collection.extend({
    model: Album,
    initialize: function () {
      this.on( "change:flipped", this.ensureSingleFlipped);
    },
    ensureSingleFlipped : function(changed, value){
      if(value){
  ...
  var InfiniteAlbumList = Backbone.Collection.extend({
    model: Album,
    totalAlbums:0,
    words:'',
    url: '/mixed/11',
    ...
    checkSize: function  () {
      this.totalAlbums += 1; 
      if(this.length > 36){
        this.remove(this.first(6));
        this.trigger('reducedSize', 3);
      }
    },
    ensureSingleFlipped : function(changed, value){
      if(value){
  ...
  var PlayerView = Backbone.View.extend({
    className:'back player-container',
    tagName:'div',
    initialize: function() {
      var provider = this.model.get('provider');
      this.template = _.template($('#'+ provider + '-template').html());
      this.$el.addClass(provider);
    },
    render: function() {
  ...
  var LoginView = Backbone.View.extend({
    ...
    events: {
      "click .login-container"   : "flipAlbum",
      "click .video"   : "toggleVideo",
      "click .spotify"   : "toggleSpotify",
      "click .bandcamp"   : "toggleBandcamp"
    },
    initialize: function() {

      this.userImage = $('.login-image');
      this.userName = $('.username');
      _.bindAll(this, 'initFacebook', 'gotFacebookEvent', 'handleFacebookEvent', 
                      'fetchUserImage', 'getGoInstantToken', 'initUser', 'albumAdded','albumRemoved', 'updateUser');

      fire.on('addAlbum', this.albumAdded);
      fire.on('removeAlbum', this.albumRemoved);
    ...
    initFacebook: function(){
      ...
      window.fbAsyncInit = function() {
        ...
        FB.getLoginStatus(function(response) {
          that.gotFacebookEvent(response);
        });
      };
    },
    gotFacebookEvent: function (fbResponse) {
      var dfd = $.Deferred();
      var that = this;
      ...
      if (fbResponse.status === 'connected') {
        FB.api(
          "/me",
            ...
            if (fbResponse && !fbResponse.error) {
              that.model = new User();
              that.model.set('id', fbResponse.id);
              that.model.set('name', fbResponse.name);
              that.model.set('facebookToken', FB.getAccessToken());
              dfd.resolve(that.model);
            }
          }
        );
        ...
        FB.Event.subscribe('auth.authResponseChange', that.gotFacebookEvent);

        dfd.reject();
      }

    this.handleFacebookEvent(dfd.promise());
      
    },
    handleFacebookEvent: function(eventDeferred){
      
      var that = this;

      eventDeferred.done(function(){
        that.fetchUserImage().pipe(_.bind(that.getGoInstantToken, that)).pipe(_.bind(that.initUser, that))
          .done(function(context){
            console.log(that.model);
            that.loggedIn();
            fire.trigger('loginViewLoaded', that.model);
          });
        }).fail(function(){
          that.loggedOut();
          fire.trigger('loginViewLoaded', that.model);
      ...
      FB.api(
        ...
        function (response) {
          if (response && !response.error) {
            that.model.set('image', response.data);
            fbImageIsFetched.resolve(that);
          }
        }
      );

      return fbImageIsFetched.promise();
    },
    getGoInstantToken: function(){
      
      var goInstantInitiated = $.Deferred();
      var that = this;
      $.ajax({
        type: "POST",
        url: "/login",
        data: {id: that.model.get('id'), fbToken: that.model.get('facebookToken'), name: that.model.get('name')}
      }).done(function(response){
        that.model.set('goinstantToken', response.goinstantToken);
        goInstantInitiated.resolve(that);

      });

      return goInstantInitiated.promise();
    },
    initUser: function(context){

      var userInitiated = $.Deferred();

      var url = 'https://goinstant.net/reimertz/discover';
      context.connection = new goinstant.Connection(url);

      context.connection.connect(context.model.get('goinstantToken'), function(err) {
        if (err) {
          if (err instanceof goinstant.errors.AuthenticationError) {
            console.log('JWT was not accepted:', err);
          } else {
        ...
        context.room = context.connection.room(context.model.get('id'));

        context.room.join(function (err) {
          context.room.self().key('model').get(function(err, userModel) {
            if (err) {
              console.log(err);
            }
            if(userModel === null){
              context.room.self().key('model').set(context.model);
            }
            
            _.extend(context.model.attributes, userModel);

            context.model.set('userAlbums', new UserAlbumList(userModel.userAlbums));
            
            userInitiated.resolve(context);
          });
        });
      });
      return userInitiated.promise();
    },
    updateUser: function(model){
      this.room.self().key('model').set(model, function(err) {
        if (err) {
    ...
    loggedIn: function() {
      ...
      this.userImage.attr('src', (this.model.get('image').url));

      this.$el.toggleClass('spotify', this.model.get('spotify'));
      this.$el.toggleClass('bandcamp', this.model.get('bandcamp'));
    ...
    albumAdded: function(album){
      this.model.get('userAlbums').add(album);
      album.added();

      this.updateUser(this.model);
    },
    albumRemoved: function(album){
      this.model.get('userAlbums').remove(album);
      album.remove();

      this.updateUser(this.model);
    },
    toggleVideo: function(){
      $('#video_bg').toggleClass('show', this.model.set('easyMode', !this.model.get('easyMode')));
      this.updateUser(this.model);
    },
    toggleSpotify: function(){
      this.$el.toggleClass('spotify', this.model.set('spotify', !this.model.get('spotify')));
      
      this.updateUser(this.model);
      fire.trigger('providerChanged', this.model);

    },
    toggleBandcamp: function(){
      this.$el.toggleClass('bandcamp', this.model.set('bandcamp', !this.model.get('bandcamp')));
    ...
    flipAlbum: function() {
      this.$el.find('.login-container').toggleClass('flipped');
    }
  });
  ...
  var AlbumView = Backbone.View.extend({
    ...
    events: {
      "click .album-container"   : "flipAlbum",
      "click .add-album" : "addAlbum",
      "click .remove-album" : "removeAlbum",
      "render": "afterRender"

    },
    initialize: function() {
      this.listenTo(this.model, 'change:flipped', this.update);
      this.listenTo(this.model, 'change:added', this.update);
      this.listenTo(this.model, 'remove', this.deleteAlbumView);
    },
    render: function() {
      this.$el.html(this.template(this.model.toJSON()));

      //this.$el.css('left', 125*Math.floor((Albums.totalAlbums-1)/3));
      return this;
    },
    ...
    update: function() {
      this.$el.find('.album-container').toggleClass('flipped', this.model.get('flipped'));
      this.$el.find('.album-container').toggleClass('added', this.model.get('added'));

      if(!this.playerView && this.model.get('flipped')){
        this.createPlayer();
      }
      else if (this.playerView && !this.model.get('flipped')){
    ...
    addAlbum: function(event) {
      event.stopPropagation();
      fire.trigger('addAlbum', this.model);
    },
    removeAlbum: function(event) {
      event.stopPropagation();
      console.log('removeeeasdl;aksd;laksd');
      fire.trigger('removeAlbum', this.model);
      this.deleteAlbumView();
    },
    flipAlbum: function() {
      this.model.flip();
    },
    createPlayer: function(){
        ...
        this.$el.find('.album-container').append(this.playerView.render().el);
    },
    deleteAlbumView: function(){
      this.undelegateEvents();
      this.$el.removeData().unbind(); 
      this.remove();          
      Backbone.View.prototype.remove.call(this);
...
var SearchView = Backbone.View.extend({
    ...
    render: function() {
      this.$el.html(this.template({searh:'search'}));
      return this;
    }
  });
  ...
  var AppView = Backbone.View.extend({
    el: $("#dscvryApp"),
    events: {
      "click .discover": "renderDiscover",
      "click .saved-albums": "renderSavedAlbums"
    },
    initialize: function() {

      this.albums = new InfiniteAlbumList();
      this.loginView = new LoginView({el:'.login-container'});

      _.bindAll(this, 'initialize', 'renderDiscover', 'renderSavedAlbums', 'compensateScroll', 'fetchingAlbums', 'fetchingAlbumsDone', 'providerChanged', 'load');

      this.listenTo(this.albums, 'add', this.addOne);
      this.listenTo(this.albums, 'reducedSize', this.compensateScroll);
      this.listenTo(this.albums, 'reset', this.addAll);

      fire.on('providerChanged', this.providerChanged);
      fire.on('loginViewLoaded', this.load);

      this.infiniScroll = new Backbone.InfiniScroll(this.albums, {scrollOffset: 100, onFetch:this.fetchingAlbums, success:this.fetchingAlbumsDone});
      this.isInDiscover = true;

      this.loginView.initFacebook();

      
    },
    load: function(user){
      this.providerChanged(user);
      this.fetchAlbums();
    },
    renderDiscover: function () {
      if(this.isInDiscover) return ;

      this.isInDiscover = true;
      this.$("#album-list").removeClass('saved-albums-mode');
      this.infiniScroll.enableFetch();

      if(this.tempAlbums){
        this.$("#album-list").empty();
        this.albums.reset(this.tempAlbums);
    ...
    renderSavedAlbums: function () {
      if(!this.isInDiscover) return ;

      this.isInDiscover = false;
      this.$("#album-list").addClass('saved-albums-mode');

      this.infiniScroll.disableFetch();
      
      this.tempAlbums = this.albums.models;
      this.$("#album-list").empty();
      this.albums.reset(this.loginView.model.get('userAlbums').models);
    },
    compensateScroll: function(nrOfAlbumsRemoved){
      //$('docment').scrollTop($('docment').scrollTop() + 1000);
    },
    fetchAlbums: function(reset){
      var that = this;
      this.fetchingAlbums();
      $.when(this.albums.fetch()).done(function(){
        that.fetchingAlbumsDone();
      });
    },
    fetchingAlbums: function(){
      $('footer').addClass('loading');
      $('#search').text('Searching...');
      $('nav').animate({marginTop: 0, queue:false}, 200);
    },
    fetchingAlbumsDone: function(){
      $('footer').removeClass('loading');
      $('#search').text(' Albums based on ' + this.albums.words.join(', '));
      $('nav').animate({marginTop: 0, queue:false}, 200);
      isVisible = true;
    },
    ...
    providerChanged: function(user) {
      this.renderDiscover();
      console.log('providerChanged!');
      var fecthSpotify = user ? user.get('spotify') : true, 
          fetchBandcamp = user ? user.get('bandcamp') : true;
      //if one of them is true
      if(fetchBandcamp != fecthSpotify) {

        this.albums.removeProvider(((fetchBandcamp) ?  'spotify' : 'bandcamp'));

        this.albums.url =  (fetchBandcamp) ?  '/bc/11' : '/sp/11';
        this.infiniScroll.enableFetch();
      ...
      //both null
      else if (!fetchBandcamp) {
        this.albums.removeProvider('spotify');
        this.albums.removeProvider('bandcamp');
        this.albums.url = '/bc/0';
        $('#search').text(' No provider selected, kind of hard to search for covers then..'); 
        this.infiniScroll.disableFetch();
      }
      else {
        this.albums.url =  '/mixed/11';
        this.infiniScroll.enableFetch(); 
      }
    }
  });
